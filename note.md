...
- Wire into `toolsService.ts` using monaco's existing LSP bridge — the same APIs that power Ctrl+F12 / Ctrl+shift+F12 in the editor. The abandonment markers on lines 342–343 suggest this was scoped before; need to understand why it was dropped (possibly LSP-provider-availability concerns — fallback to `search_in_file` if no provider).
