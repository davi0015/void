...
- **Autocomplete rewrite — FIM-only, no chat fallback** — Rewrote the autocomplete pipeline from LRU-cache-based multi-prediction-type system to a simpler FIM-first architecture. Only models with `supportsFIM: true` appear in the Autocomplete dropdown and can do autocomplete; no chat fallback. Key changes:
  - **`openAI.sendFIM` enabled** — changed from `null` to `_sendOpenAICompatibleFIM`. Custom backends using OpenAI protocol can now do FIM via `/v1/completions`.
  - **`_sendOpenAICompatibleFIM` rewritten** — non-streaming (LiteLLM cache compatibility), properly awaited (original used unawaited `.then()/.catch()`), `supportsFIM` check preserved. Uses `AbortController` for cancellation.
  - **Soft abort for FIM** — `onAbort` for FIM requests only sets `_didAbort = true` without killing the HTTP stream. The result arrives in the background and is delivered via `onFinalMessage`. Suppresses intermediate `onText` callbacks but still delivers the final result.
  - **FIM results delivered after abort** — `onFinalMessage`/`onError` in `sendLLMMessage.ts` skip suppressed non-FIM results but always deliver FIM results even after abort.
  - **`prepareFIMMessage` stripped of instructions** — the old code prepended `.voidrules` content (4.5k chars) as `//` comments. 7B models confused by instruction comments produced garbage instead of code. FIM endpoint only needs raw code prefix/suffix.
  - **`cleanFIMText`** — strips chat artifacts (e.g. "Acknowledged.") that some models append to FIM responses.
  - **`stop=\n\n` for all completions** — replaced `stop=\n` (single-line) with `stop=\n\n` (double newlines). Models produce full blocks (condition + body) in one tab instead of requiring separate tabs.
  - **Leading blank lines stripped from suffix** — when suffix starts with `\n\n`, the model's first token triggers stop immediately → empty response. Stripping prevents this.
  - **Auto-close bracket handling** — VS Code auto-closes `(` with `)`. The FIM model includes `)` in its output, creating a duplicate. Two-part fix: (1) strip the first `)` from the completion text; (2) for multi-line follow-up after accepting a condition, extend the range to replace the auto-close `)` and prepend `) {` to the insert text.
  - **Suffix-line matching** — FIM models sometimes ignore the suffix and duplicate existing code below the cursor. Match generated lines against suffix lines and trim at the first match (with indentation matching for short lines like `}`).
  - **Adjusted prefix for bracket check** — after stripping auto-close `)`, remove the matching `(` from the prefix before `getStringUpToUnbalancedClosingParenthesis` so stray `)` from the model is caught as unbalanced.
  - **Skip debounce after accepting** — `_lastCompletionAccept` + skip debounce for immediate follow-up (e.g. body after `if(condition)`).
  - **`_triggerInlineCompletions`** — after accepting a completion, programmatically triggers VS Code to re-query the inline completions provider so the follow-up suggestion appears.
  - **Removed LRU cache, `_lastResult`, old matchup system** — the old architecture cached completions by prefix and tried to match them on re-query. This caused stale results (wrong completions after typing more characters). Replaced with simple request/response pattern — each keystroke gets a fresh FIM request after debounce.
...
- **Storage bloat crash fix + checkpoint feature disable** ✅ DONE — Void's renderer crashed repeatedly (exit code 5 / SIGTRAP on macOS) on startup, reload, and new window. Root cause: `state.vscdb` grew to 1.35 GB. `Storage.init()` calls `database.getItems()` which transfers ALL key-value pairs from main process to renderer via IPC into an in-memory `Map` — all-or-nothing, no filtered/partial load. The renderer blocked deserializing the massive payload → unresponsive → Electron's watchdog killed it. 905 MB of the 1.24 GB database was checkpoint messages — `role: 'checkpoint'` entries storing `entireFileCode` (full file content) for the undo/redo feature, interleaved in the same `void.chatMsg.*` keys as conversation messages. Full crash analysis in `docs/20260621-codespan-crash-analysis.md` (codespan) and `docs/designs/checkpoint-storage-refactor.md` (storage refactor).
  - **Codespan `shorten()` crash** — `shorten()` was an O(n²) function called synchronously during React rendering in `generateCodespanLink`. Replaced with `getRelativeStr()` — O(1). Commit `66282008`.
  - **Checkpoint feature disabled entirely** — not just storage separation, but all creation (`_addUserCheckpoint`, `_addToolEditCheckpoint`), navigation (`jumpToCheckpointBeforeMessageIdx`), and UI rendering (`Checkpoint` component, `isCheckpointGhost` opacity, `currCheckpointIdx` prop threading). All code commented out (not deleted) with `// checkpoint disabled` markers for future re-enablement. `CheckpointEntry` type and `CHECKPOINT_KEY_PREFIX` infrastructure preserved. Commit `197f95af`.
  - **Message storage simplified** — read loop reads `void.chatMsg.<threadId>.<n>` keys sequentially until `undefined` (new threads are contiguous). `messageCount` field removed entirely. Compaction to contiguous indices during read is crash-safe (store-first-then-delete-tail: if process crashes between store and delete, data is duplicated, not lost).
  - **Migration** — two `@deprecated` migrations handle old thread formats. Migration 1: very old format with messages array inline in metadata — splits into per-message keys, discards old checkpoint data. Migration 2: old per-message format — reads keys sequentially up to 100k iterations skipping gaps (from previous deletes), discards `role === 'checkpoint'` entries, compacts to contiguous indices, remaps `compactionBoundaryIdx` by subtracting checkpoints before boundary. Trailing cleanup scans full range without breaking on first `undefined` (compaction creates gaps). Commits `b0f6c3ce` (trailing cleanup + boundary remap), `f72b7839` (skip gaps instead of breaking on undefined).
  - **Production database cleanup** — VACUUM reduced file from 1.3 GB to 263 MB. After fixed migration: 0 checkpoint entries remaining (down from 1,867). SQLite doesn't shrink file on delete — marks pages as free for reuse; VACUUM required to reclaim space.
  - **`compactionBoundaryIdx` stale after migration** — checkpoints removed and message indices shifted down, but `compactionBoundaryIdx` still pointed to old index. `Math.min(boundaryIdx, chatMessages.length)` in `convertToLLMMessageService.ts` clamped it — all messages replaced by summary, causing context loss (190k → 160k tokens). Fixed by remapping during migration: `newBoundary = oldBoundary - checkpointsBeforeBoundary`. Only works for not-yet-migrated threads (checkpoints still present); already-migrated threads handled by the `Math.min` clamp.
  - **Checkpoint dedup fix** (committed before feature disable) — `_computeNewCheckpointInfo` compared `oldVoidFileSnapshot === voidFileSnapshot` — always false because `getVoidFileSnapshot()` returns a new object every call. Changed to `entireFileCode` string comparison. Commit `73098c6c`.
  - Files: `chatThreadService.ts` (core changes — type fields, `_readThread` migration, `_addMessageToThread`, all checkpoint methods commented out), `chatThreadServiceTypes.ts` (`CheckpointEntry` type, removed from `ChatMessage` union), `storageKeys.ts` (`CHECKPOINT_KEY_PREFIX`), `SidebarChat.tsx` (all checkpoint UI commented out), `toolsService.ts` + `convertToLLMMessageService.ts` (dead `role === 'checkpoint'` checks removed), `docs/designs/checkpoint-storage-refactor.md` (design doc). Branch: `feature/storage-optimisation`.
  - **Future**: checkpoint redesign as separate feature — turn-level grouping (group all file changes in one LLM turn into a single before/after checkpoint pair), pending turn checkpoint (mutable in-memory checkpoint that collects before-states as tools run), dedup (skip before-state if file unchanged), retention limit (cap at N checkpoints per thread). Plan in `docs/designs/checkpoint-storage-refactor.md`.
- **Approval button consistency + concurrent terminal execution** ✅ DONE — Fixed multiple inconsistencies in the tool approval system and added concurrent terminal tool execution. Three categories of fixes:
  - **Approval button consistency** — Read-only tools (read_file, ls_dir, etc.) no longer flash approval buttons during the batch pre-add window (UI checks `approvalTypeOfBuiltinToolName` before rendering buttons). Stream state transitions to `awaiting_user` synchronously with the tool_request row update, eliminating the microtask gap where buttons wouldn't appear. The `_setStreamState('idle')` after a batch that's awaiting user approval is now skipped, preventing intermediate renders that flicker buttons off and on.
...
- **Viewport trim after streaming** ✅ DONE — Old messages stayed mounted in the DOM when new messages were appended during streaming without scrolling. The scroll handler's trim logic skipped trimming when at the bottom (to avoid desyncing auto-scroll), but there was no post-stream trim trigger. Extracted the inline trim logic into a reusable `trimFromTop` callback, called it from a `useEffect` that fires when `isRunning` transitions from truthy → falsy (streaming complete). Also fixed the `useLayoutEffect` scroll compensation to call `scrollToBottom` instead of delta compensation when `isAtBottomRef.current` is true during a `mountStart` change (delta compensation left the viewport short of the bottom). Files: `SidebarChat.tsx`.
...
- Open follow-up resolved — see `replayReasoningInHistory` below.

**`replayReasoningInHistory` config flag** ✅ DONE
- Context: investigation of missing `reasoning_content` on GLM-5.1-Coding via LiteLLM/vLLM backend. The model reports `reasoning_tokens` in usage but non-deterministically drops the actual `reasoning_content` from the response (~20-40% of requests with short outputs). Confirmed as a **server-side vLLM bug** — reproducible in both streaming and non-streaming modes via standalone Python script (`voidRequestLogs/repro_reasoning_bug.py`). The bug only manifests when `text_tokens = 1` (very short answers); longer outputs always include reasoning.
- Problem: the old gate for replaying `reasoning_content` on prior assistant messages used `providerName === 'deepseek' || deltaFieldList.includes('reasoning_content')`. Since all OpenAI-compatible providers (including backend providers) have `reasoning_content` in their `nameOfFieldInDelta`, this caused **all** providers to replay reasoning in history — wasting tokens for providers that don't require it (only DeepSeek V4 needs it).
- Fix: added `replayReasoningInHistory?: boolean` to `reasoningCapabilities` in `VoidStaticModelInfo`. Set to `true` only on DeepSeek V4. The gate in `prepareMessages_openai_tools` now uses this flag instead of inferring from `nameOfFieldInDelta`. Removed unused `getProviderCapabilities` import from `convertToLLMMessageService.ts`.
...
- **RAG context for autocomplete** — Experimented with injecting LSP type context (hover + type definitions) as a comment block into the FIM prefix. This was out-of-distribution for FIM models — they're trained on `[code prefix] → [completion] ← [code suffix]`, not `[comment block with types] + [code prefix]`. Caused conservative single-line completions instead of multi-line. Reverted to import-only context, which is real code and in-distribution. Still hallucinates project-specific API method names (e.g. `voidModelService.selected` doesn't exist) because imports give type names but not method signatures. Future approaches to explore: (a) document symbol provider to include enclosing function signature + class fields as real code in the prefix, (b) recent edit history to give the model patterns the user has been using.
- ~~**Better terminal interaction**~~ — ✅ Implemented across 4 tasks:
  - **`read_terminal` tool** — ✅ new read-only tool, no approval needed. Reads scrollback buffer of any terminal (Void or user-created). Returns structured result: command history (command, exitCode, duration) from `CommandDetection` capability + raw output text + terminal status (`idle`/`running`/`exited`). Works on any terminal the agent knows about from the volatile context. Added `last_n_commands` param to filter to recent commands.
  - **Enriched `TerminalResolveReason`** — ✅ split `{ type: 'timeout' }` into `{ type: 'timeout', reason: 'inactivity' }` (likely interactive/stuck — pager, y/n prompt) vs `{ type: 'timeout', reason: 'backstop' }` (long-running, still producing output — dev server). The stringifier tells the agent which kind of timeout occurred and suggests `read_terminal` to investigate.
  - **Accurate terminal status in volatile context** — ✅ replaced blanket `"running"` for every open terminal with `idle` (shell at prompt, last command finished) / `running: <command>` (command executing) / `exited (code N)`, using `CommandDetection` data. The agent knows whether a terminal is busy before trying to read or interact.
  - **Update terminal tool descriptions** — ✅ warned against interactive commands (pagers, editors, REPLs, confirmation prompts). Expanded the existing `git diff | cat` hint into a broader pattern: pipe pagers to `cat`, run REPL code non-interactively, avoid commands that prompt for y/n.

- **Selective terminal auto-approve** — Currently all terminal commands require manual approval, which is tedious for safe repeated commands like `git status`. Plan:
  - **Copy + extend shell parser** — copy `extensions/terminal-suggest/src/fig/shell-parser/` (Fig's MIT-licensed shell parser, already vendored in the repo) into `src/vs/workbench/contrib/void/common/shellParser/`. Extend `getTopLevelCommands` to also extract `Command` nodes from `AssignmentList` (handles `FOO=bar git status`).
  - **Auto-approve matcher** — a function `shouldAutoApprove(command, allowlist)` that: (1) parses the command string into individual commands via the shell parser, (2) scans raw text for dangerous patterns (`$(`, backticks, `<(`, `>()`, `eval`, `source`, `exec`) — if found, reject regardless of allowlist match, (3) prefix-matches each split command against the allowlist, (4) auto-approves only if every command matches and no dangerous patterns found.
  - **Per-workspace settings storage** — add `terminalAutoApproveCommands: string[]` to workspace settings. Scope is per-workspace because safe commands are project-specific (`npm run dev` is meaningful in one project, nonexistent in another). Stored in `.vscode/settings.json` so it's version-controllable and shareable across a team.
