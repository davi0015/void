...
- **Better terminal interaction** — The agent's terminal interaction is limited: after `run_persistent_command` times out, the agent is blind — it can't check output, can't tell if a command finished or is stuck waiting for input, and resorts to hacks like `echo done` to probe shell state. Plan:
  - **`read_terminal` tool** — new read-only tool, no approval needed. Reads scrollback buffer of any terminal (Void or user-created). Returns structured result: command history (command, exitCode, duration) from `CommandDetection` capability + raw output text + terminal status (`idle`/`running`/`exited`). Works on any terminal the agent knows about from the volatile context.
  ...
  - **Accurate terminal status in volatile context** — replace blanket `"running"` for every open terminal with `idle` (shell at prompt, last command finished) / `running: <command>` (command executing) / `exited (code N)`, using `CommandDetection` data. The agent knows whether a terminal is busy before trying to read or interact.
  - **Update terminal tool descriptions** — warn against interactive commands (pagers, editors, REPLs, confirmation prompts). Expand the existing `git diff | cat` hint into a broader pattern: pipe pagers to `cat`, run REPL code non-interactively, avoid commands that prompt for y/n.
