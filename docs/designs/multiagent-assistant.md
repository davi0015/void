...
| **S5** | Image ownership — thread-owned image directories, copy-on-duplicate, move off the roaming profile (bugs 6, 7, 8). Governs images written from here on; the files already in `voidImages/` are left alone and migrate in S10. Also fixes bug 18, found on the same path: the selector's Duplicate persisted no message keys, so its copies came back empty | Duplicate a thread with images, delete one, the other's images still render |
