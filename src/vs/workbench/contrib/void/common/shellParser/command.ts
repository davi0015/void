...
// Extended: added AssignmentList so that `FOO=bar git status` yields
// `git status` as a command. The AssignmentList's children are Assignment
// nodes (which won't match Command) and an optional Command node at the
// end — the recursion picks up just that Command.
const statements = [
	NodeType.Program,
