...
const streamDisplayText = [
	...
	'|--------|--------|-------|-------------|-------|\n',
	'| Lexer work per frame | O(n) full string | O(delta) tail only | ~100x for 100k chars | Only tail re-lexed |\n',
	'| React components re-rendered | All tokens | 1-3 tail tokens | ~50x fewer | React.memo skips stable refs |\n',
	'| Main thread budget consumed | 15-30ms | <1ms | 15-30x | Measured via Performance tab |\n',
	'| DOM nodes in chat | All messages | All messages | No change yet | Virtualization planned |\n',
	'| Storage write cost | Full thread map | Single thread | ~N threads cheaper | Per-thread SQLite keys |\n',
	'| Storage write frequency | Every state change | Debounced 500ms | 3-5x fewer writes | Coalesced via RunOnceScheduler |\n',
	'| Table reflow during stream | Auto layout | Fixed layout | Rows skip relayout | table-layout: fixed |\n',
	'| Code block mount | On stream end | Lazy via observer | Deferred | IntersectionObserver |\n',
	'| Reasoning→text transition | DOM mount | Style flip | No reflow | display:none pre-mount |\n',
	'| Stream throttle interval | 50ms (20fps) | 100ms (10fps) | 2x less work | Editor gets more budget |\n',
	'\n### Summary\n\n',
	'The combination of incremental lexing, memoization, and deferred writes brings per-frame cost down ',
	'to near-constant time regardless of conversation length. The remaining bottleneck is DOM size — with ',
	'hundreds of messages in the tree, any layout-triggering change (resize, tab switch) is expensive. ',
	'Message-level virtualization would address this but requires careful scroll management.\n',
].join('')
